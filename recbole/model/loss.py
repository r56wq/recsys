...
class InfoNCELoss(nn.Module):
    ...
    def forward(self, z_i, z_j):
        ...
        Args:
            z_i (torch.tensor): shape (N, T, D)
            z_j (torch.tensor): shape (N, T, D)
        Returns:
        ...
        if self.similarity_type == "cosine":
            return -torch.log(self.temperature + torch.exp(torch.nn.functional.cosine_similarity(z_i, z_j, dim=1))).mean()
        elif self.similarity_type == "dot":
            return -torch.log(self.temperature + torch.exp(torch.sum(z_i * z_j, dim=1))).mean()
        else:
